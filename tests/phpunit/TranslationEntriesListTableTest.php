...
class TranslationEntriesListTableTest extends TestCase {
	...
	/**
	 * Hides stale translation rows beyond target locale form count.
	 *
	 * @return void
	 */
	public function test_translation_column_hides_stale_rows_beyond_target_form_count() {
		$list_table = new \WP_I18nly\Admin\UI\TranslationEntriesListTable(
			array(
				array(
					'source_entry_id' => 32,
					...
					'form_labels'     => array( '*' ),
					'form_markers'    => array( '*' ),
					'form_tooltips'   => array( 'Any number' ),
					'translations'    => array(
						array(
							'form_index'  => 0,
							'translation' => '%s item',
		...
		$this->assertIsString( $html );
		$this->assertStringContainsString( 'title="Any number"', $html );
		$this->assertStringContainsString( 'data-i18nly-form-index="0"', $html );
		$this->assertStringNotContainsString( 'data-i18nly-form-index="1"', $html );
