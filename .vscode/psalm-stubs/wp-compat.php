...
if ( ! function_exists( '_x' ) ) { function _x( $text, $context, $domain = null ) { return (string) $text; } }
if ( ! function_exists( '_n' ) ) { function _n( $single, $plural, $number, $domain = null ) { return ( 1 === (int) $number ) ? (string) $single : (string) $plural; } }
...
if ( ! function_exists( 'wp_json_encode' ) ) { function wp_json_encode( $value, $flags = 0, $depth = 512 ) { return json_encode( $value, (int) $flags, (int) $depth ); } }
...
if ( ! function_exists( 'disabled' ) ) { function disabled( $disabled, $current = true, $echo = true ) { return ( $disabled == $current ) ? ' disabled="disabled"' : ''; } }
...
if ( ! function_exists( 'add_menu_page' ) ) { function add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $callback = '', $icon_url = '', $position = null ) { return ''; } }
if ( ! function_exists( 'add_submenu_page' ) ) { function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '' ) { return ''; } }
...
if ( ! function_exists( 'register_post_type' ) ) { function register_post_type( $post_type, $args = array() ) { return true; } }
...
if ( ! function_exists( 'get_posts' ) ) { function get_posts( $args = null ) { return array(); } }
...
if ( ! function_exists( 'wp_update_post' ) ) { function wp_update_post( $postarr = array(), $wp_error = false, $fire_after_hooks = true ) { return isset( $postarr['ID'] ) ? (int) $postarr['ID'] : 0; } }
if ( ! function_exists( 'wp_insert_post' ) ) { function wp_insert_post( $postarr = array(), $wp_error = false, $fire_after_hooks = true ) { return isset( $postarr['ID'] ) ? (int) $postarr['ID'] : 1; } }
if ( ! function_exists( 'wp_trash_post' ) ) { function wp_trash_post( $post_id = 0 ) { return true; } }
...
if ( ! function_exists( 'wp_send_json_success' ) ) { function wp_send_json_success( $value = null, $status_code = null, $flags = 0 ) {} }
if ( ! function_exists( 'wp_send_json_error' ) ) { function wp_send_json_error( $value = null, $status_code = null, $flags = 0 ) {} }
if ( ! function_exists( 'wp_die' ) ) { function wp_die( $message = '', $title = '', $args = array() ) { exit; } }
if ( ! function_exists( 'admin_url' ) ) { function admin_url( $path = '', $scheme = 'admin' ) { return '/wp-admin/' . ltrim( (string) $path, '/' ); } }
if ( ! function_exists( 'add_query_arg' ) ) { function add_query_arg( $key, $value = null, $url = '' ) { return (string) $url; } }
if ( ! function_exists( 'wp_safe_redirect' ) ) { function wp_safe_redirect( $location, $status = 302, $x_redirect_by = 'WordPress' ) { return true; } }
if ( ! function_exists( 'wp_nonce_field' ) ) { function wp_nonce_field( $action = -1, $name = '_wpnonce', $referer = true, $display = true ) { return ''; } }
...
if ( ! function_exists( 'load_textdomain' ) ) { function load_textdomain( $domain, $mofile ) { return true; } }
if ( ! function_exists( 'current_time' ) ) { function current_time( $type, $gmt = 0 ) { return '1970-01-01 00:00:00'; } }
if ( ! function_exists( 'get_post' ) ) { function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) { return is_object( $post ) ? $post : (object) array( 'ID' => (int) $post ); } }
if ( ! function_exists( 'wp_mkdir_p' ) ) { function wp_mkdir_p( $target ) { return true; } }
if ( ! function_exists( 'wp_delete_file' ) ) { function wp_delete_file( $file ) { return true; } }
...
if ( ! function_exists( 'wp_add_inline_script' ) ) { function wp_add_inline_script( $handle, $data, $position = 'after' ) { return true; } }
if ( ! function_exists( 'wp_parse_args' ) ) { function wp_parse_args( $args, $defaults = array() ) { return array_merge( (array) $defaults, (array) $args ); } }
if ( ! function_exists( 'is_plugin_active' ) ) { function is_plugin_active( $plugin ) { return true; } }
if ( ! function_exists( 'plugins_api' ) ) { function plugins_api( $action, $args = array() ) { return (object) array(); } }
